...
func SetupRouter(cfg *config.Config, c *provider.Container) *gin.Engine {
	...
	apiV1 := r.Group("/api/v1")
	{
		...
		user.Use(UserJWTAuthMiddleware(cfg.UserJWT.SecretKey, c.UserRepo))
		{
			...
			user.GET("/affiliate/dashboard-v2", publicHandler.GetZhengyeDashboard)
			user.GET("/affiliate/stats", publicHandler.GetZhengyeStats)
			user.GET("/affiliate/rank", publicHandler.GetZhengyeRank)
			...
			user.GET("/affiliate/contact", publicHandler.GetZhengyeContact)
		...
		admin := apiV1.Group("/admin")
		{
			...
			authorized := admin.Use(JWTAuthMiddleware(cfg.JWT.SecretKey, c.AdminRepo), AdminRBACMiddleware(c.AuthzService))
			{
				...
				authorized.GET("/affiliate/level-health", adminHandler.GetAffiliateLevelHealth)
				...
				authorized.GET("/affiliate/promotion-plan", adminHandler.GetPromotionPlan)
				authorized.POST("/affiliate/promotion-plan", adminHandler.CreateOrUpdatePromotionPlan)
				authorized.GET("/affiliate/promotion/subordinates", adminHandler.ListPromotionSubordinates)
				authorized.GET("/affiliate/promotion/stats", adminHandler.GetPromotionStats)

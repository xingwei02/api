...
// PromotionPlanService 推广方案服务（区别于已有的 PromotionService 活动价服务）
type PromotionPlanService struct {
	db *gorm.DB
}

// NewPromotionPlanService 创建推广方案服务实例
func NewPromotionPlanService(db *gorm.DB) *PromotionPlanService {
	return &PromotionPlanService{db: db}
}

// GetPromotionPlan 获取用户的推广方案
func (s *PromotionPlanService) GetPromotionPlan(userID uint) (*models.PromotionPlan, error) {
...
func (s *PromotionPlanService) CreateOrUpdatePromotionPlan(plan *models.PromotionPlan) error {
	if plan.UserID == 0 {
...
func (s *PromotionPlanService) InitializeUserLevel(userID, parentUserID uint) error {
...
func (s *PromotionPlanService) RecordCycleData(cycleData *models.CycleData) error {
	return s.db.Create(cycleData).Error
}

// GetUserPromotionLevel 获取用户等级
func (s *PromotionPlanService) GetUserPromotionLevel(userID uint) (*models.UserPromotionLevel, error) {
...
// CheckUpgrade 检查升级条件
func (s *PromotionPlanService) CheckUpgrade(userID uint) (bool, error) {
...
func (s *PromotionPlanService) UpdateUserLevel(userID uint, newLevel int, newRate float64) error {
...
func (s *PromotionPlanService) CalculateCommission(orderAmount float64, userRate float64) float64 {
	return orderAmount * userRate / 100
}

// GetUserProgress 获取用户进度
func (s *PromotionPlanService) GetUserProgress(userID uint) (map[string]interface{}, error) {
	userLevel, err := s.GetUserPromotionLevel(userID)
	if err != nil {
		return nil, err
	}
